...
echo ""
mvn clean package -Dskip.mavenmixin-jettywar -o >/dev/null
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
...
echo ""
mvn clean deploy -Pdanhaywood-mavenmixin-sonatyperelease -Dskip.mavenmixin-jettywar -Dpgp.secretkey=keyring:id=$KEYID -Dpgp.passphrase="literal:$PASSPHRASE"
if [ $? != 0 ]; then
    echo "... failed" >&2
    exit 1
fi
